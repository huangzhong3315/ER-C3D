...
class _RPN(nn.Module):
    ...
    def __init__(self, din, out_scores=False):
        ...
        # self.RPN_Conv2 = nn.Conv3d(512, 512, kernel_size=(3, 5, 5), stride=(1, 2, 2), padding=(1, 0, 0), bias=True)
        self.RPN_Conv1 = nn.Conv3d(self.din, 512, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1), bias=True)
        ...
        # self.RCNN_attention = NONLocalBlock3D(512, inter_channels=self.RPN_output_pool)
        ...
        self.RPN_anchor_target = _AnchorTargetLayer(self.feat_stride,
                                                    self.anchor_scales)  # (tx)0:(1, 1, 960, 1, 1) 1:(1, 20, 96, 1, 1) 2:(1,20,96,1,1) 3:(1,20,96,1,1) 2分类 多分类好像是窗口回归 窗口回归(内窗口) 窗口回归(外窗口)
    ...
    def forward(self, base_feat, gt_twins):  # [(1, 512, 96, 7, 7), (1, 20, 3)] gt_twins前两列代表起止帧,第三列代表标签
        ...
        rcnn_attention = self.RCNN_attention(base_feat)
        ...
        rpn_conv1 = F.relu(self.RPN_Conv1(rcnn_attention), inplace=True)
        rpn_conv2 = F.relu(self.RPN_Conv2(rpn_conv1), inplace=True)
        print("rpn_conv2", rpn_conv2.size())
        rpn_conv3 = F.relu(self.RPN_Conv3(rpn_conv2), inplace=True)
        ...
        # rpn_attention = self.RCNN_attention(rpn_output_pool)

        # rpn_conv_up = F.relu(self.Conv_up(rpn_output_pool), inplace=True)  # tx(3)  (1,512,128,1,1)
        ...
        if self.out_scores:
            rois, rois_score = self.RPN_proposal((rpn_cls_prob.data, rpn_twin_pred.data,
                                                  cfg_key))  # (1,2000,3)[其中(1,<960,3)<960的部分是前景,第一列全0存放未来的21类标签,后两列是可能的前景的起止帧;(960,2000)的部分全0,可能代表背景]
        else:  # (回归)
            rois = self.RPN_proposal((rpn_cls_prob.data, rpn_twin_pred.data, cfg_key))
        ...
        if self.training:
            ...
            rpn_cls_score = rpn_cls_score_reshape.permute(0, 2, 3, 4, 1).contiguous().view(batch_size, -1,
            ...
            rpn_keep = Variable(
                self.rpn_label.view(-1).ne(-1).nonzero().view(-1))  # (256)非0元素所在所有列(索引),即所有1和-1的索引(好像是1和0)(与前景有关)
            rpn_cls_score = torch.index_select(rpn_cls_score.view(-1, 2), 0,
                                               rpn_keep)  # (256,2)256个1和-1(好像是1和0)(前景和无关项(可能的前景))对应的前景和背景概率(以rpn_keep(256)的值为索引,取出256个rpn_cls_score的第0维(行))
            rpn_label = torch.index_select(self.rpn_label.view(-1), 0,
                                           rpn_keep.data)  # (256)256个1和-1(好像是1和0)(前景和无关项(可能的前景))对应的labels(相当于去除了背景0)
            ...
            self.rpn_loss_twin = _smooth_l1_loss(rpn_twin_pred, rpn_twin_targets, rpn_twin_inside_weights,
                                                 rpn_twin_outside_weights, sigma=3,
                                                 dim=[1, 2, 3, 4])  # 平滑的L1损失函数 预测回归系数与目标回归系数之间的距离
