...
from lib.model.tdcnn.drsn import drsn_tdcnn
...
def parse_args():
    ...
    parser.add_argument('--net', dest='net', default='res34', type=str, choices=['c3d', 'c3d', 'res34', 'res50', 'drsn34'],
...
def train_net(tdcnn_demo, dataloader, optimizer, args):
    ...
    for step, (video_data, gt_twins, num_gt) in enumerate(dataloader):
        ...
        # b = 0.5
        # c = 0.5
        # loss = a * (b * rpn_loss_cls.mean() + (1-b) * rpn_loss_twin.mean()) \
        #     + (1-a) * (c * RCNN_loss_cls.mean() + (1-c) * RCNN_loss_twin.mean())

        a = 0.5
        b = 0.5
        c = 0.5
        loss = a * (b * rpn_loss_cls.mean() + (1 - b) * RCNN_loss_cls.mean()) \
               + (1 - a) * (c * rpn_loss_twin.mean() + (1 - c) * RCNN_loss_twin.mean())
...
if __name__ == '__main__':
    ...
    elif args.net == 'drsn34':
        tdcnn_demo = drsn_tdcnn(pretrained=False)
    else:
        print("network is not defined")

    tdcnn_demo.create_architecture()   # c3d_tcnn
    print(tdcnn_demo)
